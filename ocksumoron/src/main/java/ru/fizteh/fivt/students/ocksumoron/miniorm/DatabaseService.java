...
public class DatabaseService<T> implements Closeable {
    ...
    void createTable() throws SQLException {
        ...
        for (int i = 0; i < fields.length; ++i) {
            ...
            queryBuilder.append(getColumnName(fields[i]) + " " + H2StringsResolver.resolve(fields[i].getType()));
            if (i == pkIndex) {
    ...
    public void insert(T record) throws SQLException, IllegalAccessException {
        ...
        for (int i = 0; i < fields.length; ++i) {
            ...
            queryBuilder.append(getColumnName(fields[i]) + " ");
        }
    ...
    public void delete(T record) throws IllegalArgumentException,
        ...
        StringBuilder queryBuilder = new StringBuilder();
        queryBuilder.append("DELETE FROM " + tableName + " WHERE " + fields[pkIndex].getName() + " = ?");
    ...
    public void update(T record) throws IllegalArgumentException,
        ...
        queryBuilder.append(" WHERE " + getColumnName(fields[pkIndex]) + " = ?");
    ...
    public <K> T queryById(K key) throws IllegalArgumentException,
            SQLException {
        StringBuilder queryBuilder = new StringBuilder();
        queryBuilder.append("SELECT * FROM " + tableName + " WHERE " + fields[pkIndex].getName() + " = ?");
        try (Connection conn = pool.getConnection()) {
